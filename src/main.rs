...
async fn get_rank_list(contest_id: web::Path<i32>, rule: web::Query<oj::RankRule>, config: web::Data<Config>) -> impl Responder {
    ...
    user_list.sort_by(|a, b| {
        ...
        let a_tuple =get_score_list(&a_list, rule, config.deref());
        let a_score: f64 = a_tuple.0.iter().sum();
        let b_tuple =get_score_list(&b_list, rule, config.deref());
        let b_score: f64 = b_tuple.0.iter().sum();
        let order = compare_users(&b_list, &a_list, (b_score, a_score), (b_tuple.1, a_tuple.1), rule) ;
    ...
    let scores = get_score_list(&list, rule, config.deref()).0;
    rank.push(UserRank {
    ...
    for i in 1..user_list.len() {
        ...
        let f= get_score_list(&former_list, rule, config.deref());
        let f_score:f64=f.0.iter().sum();
        let f_index=f.1;
        let n = get_score_list(&now_list, rule, config.deref());
        let n_score: f64 = n.0.iter().sum();
        let n_index=n.1;
        let scores = get_score_list(&now_list, rule, config.deref()).0;

        if Ordering::Equal == compare_users(&former_list, &now_list, (f_score, n_score), (f_index,n_index),rule) {
